...
public class SaxBookParser implements BookParser {
	...
	public String serialize(List<Book> books) throws Exception {
		...
		transformer.setOutputProperty(OutputKeys.ENCODING, "UTF_8");// 设置输出采用的编码方式
        ...
        handler.startDocument();  //开始生成文档
        handler.startElement(uri, localName, "books", null); //开始最外层父元素 books的生成
        ...
        for(Book book : books){
        	...
        	handler.endElement(uri, localName, "name");//name元素结束
        	...
        	handler.endElement(uri, localName, "price");//price元素结束
        	
        	handler.endElement(uri, localName, "book"); //book元素结束
        	
        }
		
		handler.endElement(uri, localName, "books"); //books 父元素节点结束
		handler.endDocument();// 文档已到末尾 结束
	...
	private class MyHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName,
			...
			if(localName.equals("book")){//新的book节点开始 则从新设置一个book对象
				book = new Book();
			}
		...
		public void endElement(String uri, String localName, String qName)
			...
			if(localName.equals("id")){//读取文本内容
				book.setId(Integer.parseInt(builder.toString()));
			}else if(localName.equals("name")){//读取文本内容
				book.setName(builder.toString());
			}else if(localName.equals("price")){//读取文本内容
				book.setPrice(Float.parseFloat(builder.toString()));
			}else if(localName.equals("book")){//book元素结尾 则读取完毕
				books.add(book);
			}
		}
